        ...
        if ACTIVE_AGENTS > 0:
            for i in range(ACTIVE_AGENTS):
                ...
                if T_odom_human_transform is None:
                    self.get_logger().warn(f"Can't find human pose for {human_frame_name}. Using default/last known for this cycle.", throttle_duration_sec=1.0, throttle_time_source_type='RCL_STEADY_TIME')
