...
def generate_launch_description():
    ...
    try: tiago_gazebo_pkg_share_dir = get_package_share_directory('tiago_gazebo')
    except PackageNotFoundError: print("[ERROR] Package 'tiago_gazebo' not found.")
    ...
    hallway_params = {'hallway_length':15.0,'hallway_width':5.0,'wall_thickness':0.1,'wall_height':2.5,'wall_mesh_path':'package://sm_mppi_planner/models/wall/wall.dae'}
    goal = {'x': 7.0, 'y': 0.0}
    planner_params = {'static_cost_weight': 25.0, 'safety_boundary': 0.2}

    humans_yaml_string = """
- {type: standing, x: 0.0, y: 1.5, vx: 0.0, vy: 0.0}
- {type: standing, x: 0.5, y: 1.2, vx: 0.0, vy: 0.0}
...
"""
    ...
    human_publisher_base_params={'humans_yaml':humans_yaml_string,'human_radius':0.3,'x_limits':[-(hallway_params['hallway_length']/2), (hallway_params['hallway_length']/2)],'y_limits':[-(hallway_params['hallway_width']/2), (hallway_params['hallway_width']/2)]}
    ...
    hw=hallway_params['hallway_width']/2.0; hl=hallway_params['hallway_length']/2.0; wt=hallway_params['wall_thickness']/2.0; sb=planner_params['safety_boundary']
    ...
    ld=LaunchDescription(); ld.add_action(use_sim_time_arg); ld.add_action(launch_rviz_arg); ld.add_action(launch_gazebo_arg)
