...
class FakeHumanPublisher(Node):
    ...
    def update_and_publish(self):
        ...
        for human_idx, human in enumerate(self.humans):
            ...
            hpv_msg.position.x = float(human['pos'][0])
            hpv_msg.position.y = float(human['pos'][1])
            hpv_msg.position.z = 0.0
            hpv_msg.velocity.x = float(human['vel'][0])
            hpv_msg.velocity.y = float(human['vel'][1])
            ...
            marker.pose.position.x = float(human['pos'][0])
            marker.pose.position.y = float(human['pos'][1])
            ...
            start_point = Point(x=float(human['pos'][0]), y=float(human['pos'][1]), z=1.1)
            vel_scale = 0.5
            if np.linalg.norm(human['vel']) > 0.1:
                end_point = Point(
                    x=start_point.x + float(human['vel'][0]) * vel_scale,
                    y=start_point.y + float(human['vel'][1]) * vel_scale,
                    z=start_point.z
                )
